  ...
  #All mothers are known:
    ...
    if(!all(Lynx.Table[Lynx.Table$YOB==juv.year,mother] %in% Lynx.Table$ID)){
      ...
      if(length(mom.present[mom.present == FALSE])==1){
        txt.juv<-paste("Le juvénile ", Lynx.Table[Lynx.Table$YOB==juv.year,ID], " a été observé sans sa mère ", Lynx.Table[Lynx.Table$YOB==juv.year,mother],".",sep="")
      }else{
...
if(Language== "DE"){
  ...
  if(length(Lynx.Table[Lynx.Table$YOB==juv.year & Lynx.Table$mother=="",ID])>0){
    ...
    if(!all(Lynx.Table[Lynx.Table$YOB==juv.year,mother] %in% Lynx.Table$ID)){
      ...
      if(length(mom.present[mom.present == FALSE])==1){
        txt.juv<-paste("Der juvenile ", Lynx.Table[Lynx.Table$YOB==juv.year,ID], " wurde ohne seine Mutter (",Lynx.Table[Lynx.Table$YOB==juv.year,mother],") beobachtet.",sep="")
      }else{
...
if(JuvasMother==FALSE){
  cat("**Da sich die Jungluchse zwischen Februar und April häufig von ihren Müttern trennen, werden die Mütter bei Durchgängen, die während dieser Zeit (zweite Winterhälfte) stattfinden, nicht als ebenfalls erfasst gezählt.** 
...
cat("Die potenzielle Anzahl der Fallennächte lag bei ",prettyNum(potentially.TN, big.mark="'"),". **Diebstahl, Vandalismus, technische Probleme, Bedienungsfehler und Schneefall** reduzierten den Aufwand der Probeentnahme auf ",prettyNum(effective.TN, big.mark="'")," tatsächliche Fallennächte, was ",gsub("[.]",",",sprintf("%.1f",PercentTN)),"% des Potenzials entspricht.",ValuePercentTN," **Die positiven Standorte sind gleichmässig über das gesamte Untersuchungsgebiet verteilt (Abb. 3) (Beschreibung der Verteilung).** 
...
cat("**Während dieser Durchgang wurde(n) der/die Luchs(e) ", paste(Seen.external, collapse = ", "), "nur ausserhalb des 60-Tage-Zeitraums entdeckt. Diese Luchse erscheinen auf der Karte (Abb. 3), werden aber in den übrigen Analysen nicht berücksichtigt. Diese(r) Luchs(e) werden bei der Modellunsicherheit berücksichtigt.**
...
model.descritption<-data.table(Model=c("0","b","h","t","bh"),
                      ...
                      "**bei einerseits dem davon ausgegangen wird, dass die Fangwahrscheinlichkeit aufgrund einer biologischen Reaktion variiert**", #Mb
                      "**welches unterschiedliche individuelle Fangwahrscheinlichkeiten zulässt**", #Mh
                      "**bei einerseits dem davon ausgegangen wird, dass die Wahrscheinlichkeit des Fangens mit der Zeit variiert**",#Mt
                      "**bei einerseits dem davon ausgegangen wird, dass Individuen unterschiedliche Fangwahrscheinlichkeiten haben, die sich nach ihrem ersten Fang ändern**")) #Mbh

model.descritption<-model.descritption[model.descritption$Model==substring(Mark.Table.inline[which.best.model.selected,Model], 2),Description]


cat("Das Modell ",paste("M~",substr(Mark.Table.inline[which.best.model.selected,Model], 2, 3),"~",sep=""),", ",model.descritption,", erklärt die Daten am besten. Die resultierende geschätzte Abundanz (95% Konfidenzintervall) anhand dieses Fang-Wiederfang-Modells war ",
...
Plot_KORA_Report(Species.other[i])
 ...
 cat("![**Abb. A.",i,".** Nachweise ",Species.other.table[Species.other.table$Latin==Species.other[i],DE]," (*",Species.other[i],"*) im Referenzgebiet ",Text.table[1,Refarea+1]," (blaues Polygon) im Teil-Kompartiment ",names(Text.table)[Refarea+1],".  Weisse Kreise mit einem schwarzen Punkt: Standorte mit ",Species.other.table[Species.other.table$Latin==Species.other[i],DE],"; weisse Kreise ohne Punkt: Standorte ohne ",Species.other.table[Species.other.table$Latin==Species.other[i],DE],".
