...
result.date<-"2023-02-24" #Date quand Ines met dans la base de donnée (Result_date)
Email.Type<-"species" #"species" "genotype"
communication.date<-"2023-02-28"
...
for(i in 1:length(unique(pts$Nummer))){
  ...
  file<-paste(result.date,"/w_", Email.Type,"_genetic_compartment_",unique(pts$Nummer)[i],"_",communication.date,".xlsx",sep="")
  ...
  for(i in 1:length(unique(pts$Nummer))){
    ...
    if(unique(pts$Nummer)[i] %in% c("I","IV")){
      my_email <- my_outlook$create_email(body.FRDE,
                                          ...
                                          to = c("l.legrand@kora.ch","i.moreno@kora.ch"
                                                 #,"r.manz@kora.ch"
                                                 ),send_now=FALSE)
      ...
      if(Email.Type=="genotype"){
        file<-paste(result.date,"/w_genotype_genetic_compartment_",unique(pts$Nummer)[i],"_",communication.date,".xlsx",sep="")
      }
      if(Email.Type=="species"){
        file<-paste(result.date,"/w_species_genetic_compartment_",unique(pts$Nummer)[i],"_",communication.date,".xlsx",sep="")
      }
  ...
  for(i in 1:length(unique(pts$Nummer))){
    ...
    if(unique(pts$Nummer)[i] %in% c("I","IV")){
      my_email <- my_outlook$create_email(body.FRDE,
                                          subject = paste(subject.FR," ",subject.DE," Komp-",unique(pts$Nummer)[i]," (",
                                                          paste(unique(Table[Table$Kompartiment_Compartiment==unique(pts$Nummer)[i],"Kanton_Canton"]),collapse = "/"),")", sep=""),
                                          to = c("l.legrand@kora.ch", "i.moreno@kora.ch"),send_now=FALSE)
