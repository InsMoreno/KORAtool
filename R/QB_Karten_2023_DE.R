...
startDate <- "2023-02-01" #Startdatum (Format yyy-mm-dd) immer in ""
...
title_map1 <- "Wolfsnachweise in der Schweiz" #map with packs, pairs & single wolves
title_map2 <- "Rudel und Paare in der Schweiz" #map with packs & pairs
title_map3 <- "Rudel in der Schweiz" #map with packs
title_map4 <- "Paare in der Schweiz" #map with pairs
...
packs$packID <- as.factor(packs$packID)
packs <- packs %>% 
...
ggsave(paste0("DE_Rudel_cantons_leg_", Sys.Date(), ".jpg"), Plot_pack_cantons, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("DE_Rudel_comp_leg_", Sys.Date(), ".jpg"), Plot_pack_comp, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("DE_Rudel_cantons_comp_leg_", Sys.Date(), ".jpg"), Plot_pack_cant_comp,
       width = 12, height = 8, dpi = 500)
...
# Save the ggdraw as a file
ggsave(paste0("DE_Rudel_cantons_NOleg_", Sys.Date(), ".jpg"), Plot_pack_cantons_NOleg, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("DE_Rudel_comp_NOleg_", Sys.Date(), ".jpg"), Plot_pack_comp_NOleg, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("DE_Rudel_cantons_comp_NOleg_", Sys.Date(), ".jpg"), Plot_pack_cant_comp_NOleg,
       width = 12, height = 8, dpi = 500)
...
# 
# # Save the ggdraw as a file
# ggsave(paste0("DE_Paare_canton_leg_", Sys.Date(), ".jpg"), Plot_pair, 
#        width = 12, height = 8, dpi = 500)
# ggsave(paste0("DE_Paare_comp_leg_", Sys.Date(), ".jpg"), Plot_pair_comp, 
#        width = 12, height = 8, dpi = 500)
# 
# 
# #---------------------------------------PACK & PAIR map---------------------------------  
...
# #Plot_packPair_comp
# # Save the ggdraw as a file
# ggsave(paste0("DE_RudelundPaare_cantons_", Sys.Date(), ".jpg"), Plot_packPair_cantons, 
#        width = 12, height = 8, dpi = 500)
# ggsave(paste0("DE_RudelundPaare_comp_", Sys.Date(), ".jpg"), Plot_packPair_comp, 
#        width = 12, height = 8, dpi = 500)
# 
# ## With legend:
...
# 
# # Save the ggdraw as a file
# ggsave(paste0("DE_RudelundPaare_cantons_leg_", Sys.Date(), ".jpg"), Plot_packPair_leg_cantons, 
#        width = 12, height = 8, dpi = 500)
# ggsave(paste0("DE_RudelundPaare_comp_leg_", Sys.Date(), ".jpg"), Plot_packPair_leg_comp, 
#        width = 12, height = 8, dpi = 500)
# 
# 
...
caption_reg <- "*unabhängig von der Umsetzung"
...
custom_leg <- tm_shape(packs_map_only_df_PA_all)+
  ...
  tm_add_legend(type = "symbol",
                col = c("royalblue4", "#298bd6", rep("white", nrow(packs_map_only_df))),
                labels = c(paste0("Rudel CH (", nrow(pack_sui), ")"),
                           paste0("Rudel grenzüberschr. (", nrow(pack_transb), ")"),
                           paste0(packs_map_only_df$no," = ", packs_map_only_df$name)),
                title = "",
                border.col = "white",
...
custom_leg_reg <- tm_shape(packs_map_only_df_notPA)+
  ...
  tm_add_legend(type = "symbol",
                ...
                labels = c(paste0("Teilregulierungen: 2/3 der Jungwölfe  (", nrow(packs_map_only_df_PA_some), ")"), # \n (2/3 des louveteaux ou géniteur particulièrement nuisible)
                           paste0("vollständige Eliminierung (", nrow(packs_map_only_df_PA_all), ")"),
                           "(JSV 2023/24 - Bewilligungen gültig vom 01.12.23 bis 31.01.24)",
                           paste0("Regulierung aufgrund einer Beschwerde beim Bundesverwaltungsgericht ausgesetzt (", nrow(packs_map_only_df_Recours), ")")
                ),
                title = "Bewilligte proaktive Regulierungen, die von den Kantonen vorgesehen sind*:",
                alpha = 0.5)+
  tm_legend(legend.text.size = 0.75,
            legend.title.size = 0.9
  ) +
...
custom_leg_reg_RA <- tm_shape(packs_map_only_df_notPA)+
  ...
  tm_add_legend(type = "symbol",
                ...
                labels = c(paste0("Teilregulierungen: 1/2 bis 2/3 der Jungwölfe (", nrow(packs_map_only_df_RA), ")"), 
                           "(JSV 2023 - Bewilligungen gültig bis 31.03.24)"
                ),
                title = "Bewilligte reaktive Regulierungen, die von den Kantonen vorgesehen sind*:",
                alpha = 0.5)+
  tm_legend(legend.text.size = 0.75,
            legend.title.size = 0.9
  ) +
...
# Save the ggdraw as a file
ggsave(paste0("DE_Rudel_Regulation_cantons_", Sys.Date(), ".jpg"), Plot_pack_reg_cantons, width = 12, height = 8, dpi = 500)
ggsave(paste0("DE_Rudel_Regulation_comp_", Sys.Date(), ".jpg"), Plot_pack_reg_comp, width = 12, height = 8, dpi = 500)
ggsave(paste0("DE_Rudel_Regulation_cantons_comp_", Sys.Date(), ".jpg"), Plot_pack_reg_cant_comp, width = 12, height = 8, dpi = 500)
