...
title_map2 <- "Branchi e coppie in Svizzera" #map with packs & pairs
title_map3 <- "Branchi in Svizzera" #map with packs
title_map4 <- "Coppie in Svizzera" #map with pairs
caption <- paste0("Stato il ", formatted_endDate)
...
packs_map_only <- tm_shape(packs_map_only_df) + 
  ...
  tm_add_legend(type = "symbol",
                ...
                labels = c(paste0("Branchi (", nrow(pack_sui), ")"),
                           paste0("Branchi transfrontalieri (", nrow(pack_transb), ")"),
...
ggsave(paste0("IT_Rudel_cantons_", Sys.Date(), ".jpg"), Plot_pack_cantons, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("IT_Rudel_comp_", Sys.Date(), ".jpg"), Plot_pack_comp, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("IT_Rudel_cantons_comp_", Sys.Date(), ".jpg"), Plot_pack_cant_comp, width = 12, height = 8, dpi = 500)
...
# Save the ggdraw as a file
ggsave(paste0("IT_Rudel_cantons_NOleg_", Sys.Date(), ".jpg"), Plot_pack_cantons_NOleg, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("IT_Rudel_comp_NOleg_", Sys.Date(), ".jpg"), Plot_pack_comp_NOleg, 
       width = 12, height = 8, dpi = 500)
ggsave(paste0("IT_Rudel_cantons_comp_NOleg_", Sys.Date(), ".jpg"), Plot_pack_cant_comp_NOleg, width = 12, height = 8, dpi = 500)
...
#           shape = "type",
#           title.shape = "Stato",
#           shapes.labels = paste0("Coppie (", nrow(pair_count), ")"),
#           labels = paste0("Coppie (", nrow(pair_count), ")"),
#           legend.show = FALSE,
#           shapes = 22,
#           border.lwd = NA,
#           alpha = 0.5) +
...
# 
# # Save the ggdraw as a file
# ggsave(paste0("IT_Paare_canton", Sys.Date(), ".jpg"), Plot_pair, 
#        width = 12, height = 8, dpi = 500)
# ggsave(paste0("IT_Paare_comp", Sys.Date(), ".jpg"), Plot_pair_comp, 
#        width = 12, height = 8, dpi = 500)
# 
# 
# #-------------------------------------PACK & PAIR map-----------------------------------  
...
#           labels = c(paste0("Branchi (", nrow(pack_sui), ")"), 
#                      paste0("Branchi transfrontalieri (", nrow(pack_transb), ")"), 
#                      paste0("Coppie (", nrow(pair_count), ")")),
#           size = 5, 
#           palette = c("royalblue4", "#298bd6",  "darkorange2"), 
#           shape = "type",
#           title.shape = "Simboli",
#           shapes.labels = c("Branchi", "Coppie"),
#           shapes = c(21, 22),
#           border.lwd = NA,
#           alpha = 0.5) +
#   # tm_text("no", 
...
# 
# # Save the ggdraw as a file
# ggsave(paste0("IT_RudelundPaare_cantons_", Sys.Date(), ".jpg"), Plot_packPair_cantons, 
#        width = 12, height = 8, dpi = 500)
# ggsave(paste0("IT_RudelundPaare_comp_", Sys.Date(), ".jpg"), Plot_packPair_comp, 
#        width = 12, height = 8, dpi = 500)
# 
# ## With legend:
...
# 
# # Save the ggdraw as a file
# ggsave(paste0("IT_RudelundPaare_leg_cantons_", Sys.Date(), ".jpg"), Plot_packPair_leg_cantons, 
#        width = 12, height = 8, dpi = 500)
# ggsave(paste0("IT_RudelundPaare_leg_comp_", Sys.Date(), ".jpg"), Plot_packPair_leg_comp, 
#        width = 12, height = 8, dpi = 500)
# 
...
caption_reg <- "*indipendentemente dalla sua attuazione"
...
custom_leg <- tm_shape(packs_map_only_df_PA_all)+
  ...
  tm_add_legend(type = "symbol",
                col = c("royalblue4", "#298bd6", rep("white", nrow(packs_map_only_df))),
                labels = c(paste0("Branchi CH (", nrow(pack_sui), ")"),
                           paste0("Branchi transfrontalieri (", nrow(pack_transb), ")"),
                           paste0(packs_map_only_df$no," = ", packs_map_only_df$name)),
                title = "",
                border.col = "white",
...
custom_leg_reg <- tm_shape(packs_map_only_df_notPA)+
  ...
  tm_add_legend(type = "symbol",
                ...
                labels = c(paste0("Regolazione parziale: 2/3 dei cuccioli (", nrow(packs_map_only_df_PA_some), ")"), # \n (2/3 des louveteaux ou géniteur particulièrement nuisible)
                           paste0("eliminazione completa (", nrow(packs_map_only_df_PA_all), ")"),
                           "(OCP 2023/24 - autorizzazioni valide dal 01.12.23 al 31.01.24)",
                           paste0("Regolazione sospesa a seguito di un ricorso depositato al Tribunale amministrativo federale (", nrow(packs_map_only_df_Recours), ")")
                ),
                title = "Regolazione proattive autorizzata, previste dai cantoni e autorizzate*:",
                alpha = 0.5)+
  tm_legend(legend.text.size = 0.75,
            legend.title.size = 0.9
  ) +
...
custom_leg_reg_RA <- tm_shape(packs_map_only_df_notPA)+
  ...
  tm_add_legend(type = "symbol",
                ...
                labels = c(paste0("regolazione parziale: 1/2 - 2/3 dei cuccioli (", nrow(packs_map_only_df_RA), ")"), 
                           "(OCP 2023 - autorizzazioni valide fino al 31.03.24)"
                ),
                title = "Regolazione reattive autorizzata, previste dai cantoni e autorizzate*:",
                alpha = 0.5)+
  tm_legend(legend.text.size = 0.75,
            legend.title.size = 0.9
  ) +
...
# Save the ggdraw as a file
ggsave(paste0("IT_Rudel_Regulation_cantons_", Sys.Date(), ".jpg"), Plot_pack_reg_cantons, width = 12, height = 8, dpi = 500)
ggsave(paste0("IT_Rudel_Regulation_comp_", Sys.Date(), ".jpg"), Plot_pack_reg_comp, width = 12, height = 8, dpi = 500)
ggsave(paste0("IT_Rudel_Regulation_cantons_comp_", Sys.Date(), ".jpg"), Plot_pack_reg_cant_comp, width = 12, height = 8, dpi = 500)
